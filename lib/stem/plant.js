...
const plant = (author, seedFilePath, net) => {
    const ygg = new Yggdrash(new Yggdrash.providers.HttpProvider(net ? net : 'http://localhost:8080'));
    ...
    try {
        db.get("accounts").map("address").value().map(addr => {
            if(addr === author){
                ...
                branchFile(ygg, branchData, fileDir, body, author, tx, bodyJson);
 ...
 const branchFile = (ygg, branchData, fileDir, body, author, tx, bodyJson) => {
    let branch = {
