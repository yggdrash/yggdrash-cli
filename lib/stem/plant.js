...
const plant = (seedFilePath) => {
    ...
    if (seed.contractId) {
        ...
        exec("pwd", (error, stdout, stderr) => {
            var pwd = stdout
            var contractID
            exec(`cd ${pwd.trim()}/contract-template;./gradlew`, (error, stdout, stderr) =>{
                if(stdout){
                    exec(`cd ${pwd.trim()}/contract-template/build/classes/java/main; ls`, (error, stdout, stderr) =>{
...
const branchFormat = (seed, contractID, signature, timestamp, ygg) => {
    ...
    exec("pwd", (error, stdout, stderr) => {
        ...
        // })
        fs.writeFile(`${stdout.trim()}/${branch.symbol.toLowerCase()}.branch.json`,
        JSON.stringify(branch, undefined, '\t'), err => {
            if (err) throw err;
            console.log()
            console.log(`  ` + chalk.green(`${stdout.trim()}/${branch.symbol.toLowerCase()}.branch.json `) + `saved ✨`)
        })
