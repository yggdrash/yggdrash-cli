...
let host = config().get("node").value()[0].defaults.host
let port = config().get("node").value()[0].defaults.port
...
const node = {
    ...
    start: (pass, path) => start(pass, path, sdk),
    restart,
    status: () => nodeStatus(),
    stop,
    setConfig
...
const branch = {
    seed,
    init,
    build: (owner, password) => build(owner, password, sdk),
    deploy: () => deploy(sdk),
    getBranch: () => getBranch(sdk),
    setBranch,
    status
}
...
const rawTx = {
    transferFrom: (from, to, amount, owner, password) => transferFrom(from, to, amount, owner, password, sdk),
