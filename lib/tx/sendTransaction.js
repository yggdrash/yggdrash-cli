...
const transferFrom = (branchId, from, to, amount, net) => {
    ...
    try {
        db().get("accounts").map("address").value().map(addr => {
            if (addr === from) {
                let privatekeyEncryptedKey = db().get("accounts").find({address:from}).value().encryptedKey
 ...
 const transfer = (branchId, to, amount, net) => {
    if (db().get("accounts").find({address:coinbase()}).value() == null) {
        ...
        let privatekeyEncryptedKey = db().get("accounts").find({address:coinbase()}).value().encryptedKey
        sendTransaction(privatekeyEncryptedKey, branchId, to, amount, net)
    }
 }
