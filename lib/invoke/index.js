...
const invoke = (address, password, contractName, method, params, ygg) => {
    if (db().get("accounts").find({address: admin}).value() == null) {
        console.log(`  ` + `${chalk.red("\nPlease create a account.\n")}`)
    } else {
        let privatekey = address ? decryption(address, password).toString('hex')
                                : decryption(admin, password).toString('hex')
