...
*/
 ...
 const sendTransaction = (privatekey, body, type, ygg) => {
    ...
    let branchId = "a1d0ade6dcb5db356c2e8a5ee09d2568072eef09";
    let contractVersion = "19b9710dc8f0442d09a1b5e632f7200afe751d60";

    const password = "qwer1234!@#$";
    const { address, keystoreData } = ygg.wallet.create(password);
    
    // private key 가져옴
    pk = ygg.wallet.getPrivateKey(keystoreData, password);
    pk = ygg.utils.hexString(pk);
    ...
    // TX 생성
    const tx = new ygg.tx(header);
    // Signing
    tx.sign(pk);
    // 시리얼라이즈
    let serialize = tx.serialize(body)
    // 트랜젝션의 전송
    ygg.client.sendTransaction(serialize).then((txHash) => {console.log(txHash)});
...
const getTransaction = (txhash, ygg) => {
    ygg.client.getTransaction(branchId, txhash).then(res => {
...
const getLogs = (ygg) => {
    ...
    ygg.client.fliterLogs(branchId, "PROCESSING", 1, 100).then(res => {
        console.log(JSON.stringify(res, null, " "))
    });
}
