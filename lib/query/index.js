...
const query = async (contractVersion, method, params, ygg) => {
    if (!contractVersion) {
        contractVersion = contracts(branch)["YEED"];
    }
    if(!Object.keys(params).length) {
        params["address"] = db().get("accounts").find({address: admin}).value().address
    }
    if (contractVersion) {
        let body = {
            branchId,
