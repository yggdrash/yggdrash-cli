...
const { bodyData } = require('../stem/txBody');
const { fromPrivateKey } = require('../wallet')
const { txHeaderData } = require('../stem/txHeader');
...
const actionPlant = (pk, seedFilePath, timestamp) => {
    const fileDir = path.dirname(seedFilePath)
    let branchData = JSON.parse(fs.readFileSync(seedFilePath, 'utf8'));
    const account = fromPrivateKey(toBuffer(`0x${pk}`));
    const author = account.getAddressString();
