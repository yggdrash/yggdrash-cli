...
const build = (owner, password) => {
    ...
    exec(`find . -name "*.seed.json"`, (error, seedFile, stderr) => {
        if (!seedFile.trim()) {
            ...
            if (seed.contractId) {
                signFormat(seed, ygg, timestamp, owner, password)
            } else {
                        ...
                        if(build){
                            exec(`cd ${pwd.trim()}/contracts/build/classes/java/main;ls`, (error, classFile, stderr) =>{
                                if(classFile){
                                    ...
                                    signFormat(seed, ygg, timestamp, owner, contractID, password)
                                }
...
const signFormat = (seed, ygg, timestamp, owner, contractID, password) => {
    let Break = new Error('Break')
    try {
        db().get("accounts").map("address").value().map(addr => {
            if(addr === `${owner}`){
                let privatekey = decryption(owner, password).toString('hex')
