...
const setBranch = (allBranchId, allSymbol) => {
    ...
    inquirer.prompt([{
        ...
        default: 0
            ...
            let id = answers.branch.split(',')[1].split(':')[1].trim()
...
*/
const status = () => {
    let branch = config().get('node').value()[0].branch
    if (!branch) {
        console.log(`\n  ` + `On branch ${chalk.red('No branch')}`)
    } else {
        console.log(`\n  ` + `On branch Branch ID: ${chalk.green(branch)}`)
        console.log(`  ` + `Your branch is up to date with '${branch}'.\n`)
    }
}
