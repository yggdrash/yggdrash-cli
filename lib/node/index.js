...
const start = (node, password) => {
    ...
    exec(`cd ${node};java -jar yggdrash-gateway/build/libs/yggdrash-gateway.jar & echo $! > ${getHomePath}/.yggdrash/cli/pid.file &`, (err, stdout, stderr) =>{
        console.log(err)
        console.log(stdout)
        console.log(stderr)
    })
...
const stop = node => {
    if (fs.existsSync(`${getHomePath}/.yggdrash/cli/pid.file`)) {
        ...
        console.log('  Stopping the YGGDRASH node.')
