...
const start = (node, password) => {
    ...
    if (node) {
        ...
        exec(`cd ~/.yggdrash/cli/yggdrash;java -Dkey.path=${adminPath} -Dkey.password='${password}' -jar yggdrash-gateway/build/libs/yggdrash-gateway.jar & echo $! > ${getHomePath}/.yggdrash/cli/pid &`, (err, stdout, stderr) =>{
            console.log(stdout)
        })
    }
}
