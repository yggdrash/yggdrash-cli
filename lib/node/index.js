...
const start = (password, node) => {
    adminVerify(db().get("accounts").map("address").value()[0], password)
    // TODO: log 스트림데이터
    ...
    if (node) {
        exec(`cd ${node};java -Dkey.path=${defaultPath} -Dkey.password='${password}' -jar yggdrash-gateway/build/libs/yggdrash-gateway.jar & echo $! > ${getHomePath}/.yggdrash/cli/pid &`, (err, stdout, stderr) =>{
            console.log(stdout)
        })
    } else {
        exec(`cd ~/.yggdrash/cli/yggdrash;java -Dkey.path=${defaultPath} -Dkey.password='${password}' -jar yggdrash-gateway/build/libs/yggdrash-gateway.jar & echo $! > ${getHomePath}/.yggdrash/cli/pid &`, (err, stdout, stderr) =>{
            console.log(stdout)
        })
    }
}
